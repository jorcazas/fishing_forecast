% =====================================================================================
% Tesis de licenciatura — Javier Orcazas — ITAM, Ciencia de Datos
%
% Estructura: este archivo solo arma el documento; el contenido vive en front/ y
% sections/. Para compilar:  pdflatex final_work && pdflatex final_work  (dos pasadas,
% la segunda resuelve índices y referencias cruzadas).
%
% NOTA DE FORMATO: la portada y la declaración de derechos siguen el formato habitual
% del ITAM, pero deben confirmarse con el asesor y con Dirección Escolar antes de
% imprimir. Los datos por confirmar están marcados con TODO en front/portada.tex.
% =====================================================================================
\documentclass[12pt,letterpaper,twoside,openright]{report}

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[left=3cm,right=2.5cm,top=2.5cm,bottom=2.5cm]{geometry}
\usepackage{amsmath,amssymb}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{array}
\usepackage{float}
\usepackage{url}
\usepackage[round,authoryear]{natbib}
\usepackage[hidelinks]{hyperref}

\graphicspath{{images/}}

% --- Español: babel provee nombres y guionado (instalado el 2026-09-01; cierra B1). ---
% es-tabla: «Tabla» en vez de «Cuadro»; es-noshorthands: sin atajos que interfieran con
% el modo matemático. El único nombre que se ajusta es \bibname («Referencias» en lugar
% del «Bibliografía» de babel).
\usepackage[spanish,es-tabla,es-noshorthands]{babel}
\addto\captionsspanish{\renewcommand{\bibname}{Referencias}}

% --- Macros del dominio ---------------------------------------------------------------
\newcommand{\ue}{unidad económica}
\newcommand{\UEs}{unidades económicas}
\newcommand{\horizonte}{h}

\linespread{1.25}

% Colchón de justificación: aun con guionado, permite estirar espacios antes que dejar
% líneas salidas del margen.
\setlength{\emergencystretch}{1.5em}

% --- Flotantes: permitir más figuras/tablas por página y menos páginas en blanco. -----
\renewcommand{\topfraction}{0.9}
\renewcommand{\bottomfraction}{0.85}
\renewcommand{\textfraction}{0.1}
\renewcommand{\floatpagefraction}{0.8}
\setcounter{topnumber}{3}
\setcounter{bottomnumber}{2}
\setcounter{totalnumber}{5}
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.1}

\begin{document}

\pagenumbering{roman}
\input{front/portada}
\input{front/declaracion}
\input{front/agradecimientos}
\input{front/resumen}

\tableofcontents
\listoffigures
\listoftables

\cleardoublepage
\pagenumbering{arabic}

\input{sections/01_introduccion}
\input{sections/02_revision_capitulo}
\input{sections/03_metodologia}
\input{sections/04_datos}
\input{sections/05_resultados}
\input{sections/06_producto}
\input{sections/07_conclusiones}

\appendix
\input{sections/A_apendice_figuras_cap}
\input{sections/B_apendice_variables_cap}

\cleardoublepage
\addcontentsline{toc}{chapter}{Referencias}
\bibliographystyle{plainnat}
\bibliography{referencias}

\end{document}
