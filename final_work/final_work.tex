...
Esa hipótesis obligó también a revisar la conclusión del propio trabajo original. Al re-entrenar sobre el conjunto unido los modelos que allí ganaban ---la LSTM y el ensamble XGBoost+LSTM--- su ventaja se disuelve: con tres temporadas de entrenamiento encabezan la tabla, pero por amortiguar la serie (razón de desviaciones 1.58 frente a 2.37 del XGBoost, con la misma correlación), no por seguir su forma; y con siete temporadas quedan cerca de cinco veces por detrás del XGBoost podado por SHAP, con errores de suma de temporada de $+626\%$ y $+704\%$. El error del 12.9\% que el borrador reportaba para el ensamble era, entonces, un artefacto de aquella partición corta y no una propiedad del modelo.
